...
void move_snake(vector2di& snake_head, vector2di& vector_step, vector<vector2di>& snake_tail,
    ...
    // Console.SetCursorPosition(snake_head.x, snake_head.y);
...
// vector_step = new Vector2(1, 0);
// snake_tail.Clear();
// apple = new Vector2(15, 15);
// eaten_apples = 0;
// return;
...
//while (!game_exit)
//{
    // Assign random values to variables representing the coordinates of the apple
    //rnd_apple.X = AppleX(rnd);
    //rnd_apple.Y = AppleY(rnd);

    // Draw the game field
   // ShowMap(apple, eaten_apples);

    // Move the snake on the game field
   // MoveSnake(snake_head, vector_step, snake_tail, apple, rnd_apple, eaten_apples);
